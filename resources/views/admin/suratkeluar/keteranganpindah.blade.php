        ...
        <div class="card w-100">
            ...
            <div class="card-body hover-scroll-overlay-y">
                <form class="form-data" enctype="multipart/form-data">
                    ...
                    <div class="mb-10">
                        <label class="form-label">Data Pindah</label>
                        <div class="inputs-container">
                            <div class="data-pindah">
                                <div class="mb-5">
                                    <label class="form-label">NIK</label>
                                    <select name="nik" class="form-select form-select-nik"
                                        data-placeholder="Pilih jenis inputan">
                                    </select>
                                ...
                                <div class="mb-10">
                                    ...
                                    <input type="text" class="form-control hubungan" name="hubungan">
                                    <small class="text-danger hubungan_error"></small>
                                </div>
                            </div>
                        </div>
...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            control.overlay_form('Tambah', 'Keterangan Pindah Domisili');
        });
        ...
        //     if (type == 'add') {
        //         control.submitFormMultipartData('/admin/surat-keluar/add-ket-pindah', 'Tambah',
        //             'Keterangan Pindah Domisili',
        //             'POST');
        //     } else {
        //         let uuid = $("input[name='uuid']").val();
        //         // control.submitFormMultipartData('/admin/surat-keluar/update-ket-pindah/' + uuid,
        //         //     'Update',
        //         //     'Keterangan Pindah Domisili', 'POST');
        //     }
        // });

        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            let url = '/admin/surat-keluar/show-ket-pindah/' + $(this).attr('data-uuid');
            control.overlay_form('Update', 'Keterangan Pindah Domisili', url);

            $.ajax({
                url: url,
                method: "GET",
                success: function(res) {
                    // Fungsi untuk mengambil dan memperbarui opsi pilihan pada elemen select
                    function updateSelectOptions(selectElement) {
                        $.ajax({
                            url: '/admin/surat-keluar/show-kk/' + res.data.no_kk,
                            method: "GET",
                            success: function(resKk) {
                                $(selectElement).html("");
                                let html = "<option selected disabled>Pilih</option>";
                                $.each(resKk.data, function(x, y) {
                                    html +=
                                        `<option value="${y.nik}">${y.nik}</option>`;
                                });
                    ...
                    updateSelectOptions('.form-select-nik');

                    // Tambah inputan NIK dan Hubungan Keluarga
                    $('.tambah-btn').off('click').on('click', function(e) {
                        e.preventDefault();
                        $('.inputs-container').append(`
                            <div class="data-pindah">
                                <div class="mb-5">
                                    <label class="form-label">NIK</label>
                                    <select name="nik" class="form-select" data-placeholder="Pilih jenis inputan">
                                    </select>
                                    <small class="text-danger nik_error"></small>
                                </div>
                                <div class="mb-10">
                        ...
                        // Perbarui opsi pilihan pada elemen select yang baru ditambahkan
                        updateSelectOptions($('.inputs-container select').last());
                    });

                    // Hapus inputan NIK dan Hubungan Keluarga terakhir
                    $('.hapus-btn').off('click').on('click', function(e) {
                        e.preventDefault();
                        let inputsCount = $('.inputs-container > div').length;

                        // Pastikan ada setidaknya dua input tersisa sebelum menghapus
                        if (inputsCount > 1) {
                            $('.inputs-container > div:last-child').remove();
                        }
                    });
                    ...
                    $(document).on('submit', ".form-data", function(e) {
                        ...
                        if (type == 'update') {
                            let uuid = $("input[name='uuid']").val();
                            let formData = new FormData(); // Buat objek FormData baru

                            // Tambahkan data nomor surat dan status
                            formData.append('nomor_surat', $('#nomor_surat').val());
                            formData.append('status', $('#from_select').val());

                            // Iterasi setiap elemen input di dalam .inputs-container
                            $('.inputs-container > div').each(function(index) {
                                // Dapatkan nilai input NIK dan Hubungan Keluarga
                                var nik = $(this).find('select[name="nik"]').val();
                                var hubungan = $(this).find('input[name="hubungan"]')
                                    .val();

                                // Periksa jika kedua nilai tidak kosong dan valid
                                if (nik && hubungan) {
                                    // Tambahkan data ke dalam formData
                                    formData.append('data_pindah[' + index + '][nik]',
                                        nik);
                                    formData.append('data_pindah[' + index +
                                        '][hubungan]', hubungan);
                                }
                            });

                            control.submitForm(
                                '/admin/surat-keluar/update-ket-pindah/' + uuid,
                                'Update',
                                'Keterangan Pindah Domisili', 'POST', formData);
                        }
                    });
                },
                error: function(xhr) {
        ...
        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/admin/surat-keluar/delete-ket-pindah/' + $(this).attr('data-uuid');
        ...
        $(document).on('click', '#button-cetak', function(e) {
            ...
            window.open(`/admin/surat-keluar/export-ket-pindah/` + uuid);
        });
        ...
        const initDatatable = async () => {
            ...
            $('#kt_table_data').DataTable({
                ...
                processing: true,
                ajax: '/admin/surat-keluar/get-ket-pindah',
                columns: [{
                    data: null,
                    ...
                    data: 'klarifikasi_pindah',
                    className: 'text-center',
                }, {
                    data: 'keluarga_pindah',
                    className: 'text-center',
                }, {
                    data: 'no_telepon',
                    className: 'text-center',
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        result =
                            `
                                <!--begin::Overlay-->
                                <a class="d-block overlay fancybox" data-fancybox="lightbox-group" href="{{ asset('/storage/ket_pindah/${data}') }}">
                                    <!--begin::Image-->
                                    <div class="overlay-wrapper bgi-no-repeat bgi-position-center bgi-size-cover card-rounded min-h-175px"
                                        style="background-image:url('/storage/ket_pindah/${data}')">
                                    </div>
                                    <!--end::Image-->
                ...
                columnDefs: [{
                    ...
                    render: function(data, type, full, meta) {
                        if (full.nomor_surat && full.status == "verifikasi") {
                            return `
                                ...
                                <a href="javascript:;" type="button" data-uuid="${data}" data-label="Keterangan Pindah Domisili" class="btn btn-danger button-delete btn-icon btn-sm">
                                    <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <a href="javascript:;" type="button" data-uuid="${data}" data-label="Keterangan Pindah Domisili" id="button-cetak" class="btn btn-warning btn-icon btn-sm">
        ...
        $(function() {
            control.push_select3(verifikasi, '#from_select');
            initDatatable();
        });
    </script>
@endsection
