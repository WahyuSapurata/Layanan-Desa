                                ...
                                <table id="kt_table_data"
                                    ...
                                    <thead class="text-center">
                                        <tr class="fw-bolder fs-6 text-gray-800">
                                            ...
                                            <th>Nama</th>
                                            <th>NIK</th>
                                            <th>Nama Ortu/Kepala Keluarga</th>
                                            <th>No Telepon</th>
                                            <th>Foto KTP</th>
                                            <th>Status</th>
                                            <th>No Surat</th>
...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            control.overlay_form('Tambah', 'Keterangan Tidak Mampu');
        });
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            if (type == 'add') {
                control.submitFormMultipartData('/admin/surat-keluar/add-ket-tidak_mampu', 'Tambah',
                    'Keterangan Tidak Mampu',
                    'POST');
            } else {
                let uuid = $("input[name='uuid']").val();
                control.submitFormMultipartData('/admin/surat-keluar/update-ket-tidak_mampu/' + uuid,
                    'Update',
                    'Keterangan Tidak Mampu', 'POST');
            }
        });

        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            let url = '/admin/surat-keluar/show-ket-tidak_mampu/' + $(this).attr('data-uuid');
            control.overlay_form('Update', 'Keterangan Tidak Mampu', url);
        });

        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/admin/surat-keluar/delete-ket-tidak_mampu/' + $(this).attr('data-uuid');
        ...
        $(document).on('click', '#button-cetak', function(e) {
            ...
            window.open(`/admin/surat-keluar/export-ket-tidak_mampu/` + uuid);
        });
        ...
        const initDatatable = async () => {
            ...
            $('#kt_table_data').DataTable({
                ...
                processing: true,
                ajax: '/admin/surat-keluar/get-ket-tidak_mampu',
                columns: [{
                    data: null,
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        result =
                            `
                                <!--begin::Overlay-->
                                <a class="d-block overlay fancybox" data-fancybox="lightbox-group" href="{{ asset('/storage/ket_tidak_mampu/${data}') }}">
                                    <!--begin::Image-->
                                    <div class="overlay-wrapper bgi-no-repeat bgi-position-center bgi-size-cover card-rounded min-h-175px"
                                        style="background-image:url('/storage/ket_tidak_mampu/${data}')">
                                    </div>
                                    <!--end::Image-->
                    ...
                    render: function(data, type, full, meta) {
                        if (full.nomor_surat && full.status == "verifikasi") {
                            return `
                                ...
                                <a href="javascript:;" type="button" data-uuid="${data}" data-label="Keterangan Tidak Mampu" class="btn btn-danger button-delete btn-icon btn-sm">
                                    <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <a href="javascript:;" type="button" data-uuid="${data}" data-label="Keterangan Tidak Mampu" id="button-cetak" class="btn btn-warning btn-icon btn-sm">
